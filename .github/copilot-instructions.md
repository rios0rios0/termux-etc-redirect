...
make test     # Run Tier 1 unit tests, Tier 2 integration + faccessat2 SIGSYS + multithreaded clone-race + reentrancy tests, Tier 3 integration + reentrancy + LD_PRELOAD parking tests (LD_PRELOAD is set to the Tier 1 library for that run so the assertion is exercised)
...
- **Tier 3** (`src/termux-etc-mount.c` → `termux-etc-mount`): Narrow seccomp `user_notif` supervisor for dynamic musl binaries. Same BPF filter as Tier 2 but no ptrace and no SIGSYS rewriting. Composes cleanly with `strace`/`gdb` and nested wrappers. Same reentrancy guard as Tier 2, checked before `$PREFIX` is parsed. Parks `LD_PRELOAD` in `TERMUX_ETC_LD_PRELOAD` immediately before `execve` (supervised path and short-circuit alike): the musl loader treats a bionic preload it cannot relocate as fatal, and a bionic descendant restores the shims from the parked copy in its shell rc file.
...
- `TERMUX_ETC_LD_PRELOAD` is a **contract**, not an implementation detail: Tier 3 parks `LD_PRELOAD` under that name and shell rc files (the `rios0rios0/dotfiles` `.zshenv`, whose Claude wrapper parks under the same name itself) restore it from there. Renaming it breaks every rc file that restores it.
...
- `test/test-mount.c`: Integration tests for Tier 3 — validates `/etc/resolv.conf` redirect, inherited-filter presence, unrelated-path passthrough, `LD_PRELOAD` parking (the target never sees it; `TERMUX_ETC_LD_PRELOAD` carries it), and reentrancy guard.
- `make test` runs all three tiers' tests plus smoke tests (`cat /etc/resolv.conf` under both Tier 2 and Tier 3). The SIGSYS tests run under `sigsys_launcher` so the full suite passes even from inside a wrapped shell. The Tier 3 run sets `LD_PRELOAD` to the Tier 1 library so the parking assertion is exercised rather than skipped, and a second check runs `sh -c` under the wrapper to prove the parked value reaches a bionic child intact.
