...
- **LD_PRELOAD parking.** `park_ld_preload()` copies `LD_PRELOAD` to `TERMUX_ETC_LD_PRELOAD` and removes it immediately before `execve`, on both the supervised path and the reentrancy short-circuit. The musl loader treats a bionic preload entry it cannot relocate as fatal (`Error relocating ...: symbol not found`, before `main()`), so the target must start without Termux's shims — but unsetting the variable outright left every bionic descendant (the shell behind a Claude tool call, `make`, their scripts) without `termux-exec`'s shebang rewriting, so each `#!/usr/bin/env` script failed with exit 127. A descendant restores the value from the parked copy in its shell rc file. `test/test-mount.c` asserts the target never sees `LD_PRELOAD`, and `make test` sets it to the Tier 1 library so the assertion is exercised rather than skipped.
...
- **`TERMUX_ETC_LD_PRELOAD` is a contract, not an implementation detail.** Tier 3 parks `LD_PRELOAD` under that name and shells restore it from there; the `rios0rios0/dotfiles` Claude wrapper parks under the same name itself, so a wrapper and a redirector of different vintages still agree. Renaming it breaks every rc file that restores it.
